...
function hideDetail() { selectedFeature = null; $("detail-card").classList.remove("compact-note-editor", "compact-symbol-editor"); $("detail-card").hidden = true; $("detail-photo").src = ""; $("detail-link").hidden = true; $("symbol-editor").hidden = true; $("text-editor").hidden = true; }
...
$("save-text-note").addEventListener("click", () => { const type = selectedFeature?.get("noteType"); if (type !== "text" && type !== "sketch") return; const title = $("edit-note-title").value.trim(); const body = $("edit-note-text").value.trim(); const targetType = type === "text" ? $("edit-note-type").value : "text"; if (type === "text" && targetType !== "text") { const anchor = selectedFeature.get("bubbleAnchor"); if (anchor) selectedFeature.setGeometry(new ol.geom.Point(anchor)); selectedFeature.setProperties({ noteType: "symbol", bubble: undefined, bubbleAnchor: undefined, bubbleEnd: undefined, symbolType: targetType, symbolText: body || title, rotation: 0, title: symbolNames[targetType], body: body || title || "Kartfestet symbol" }); saveData(); notesSource.changed(); hideDetail(); toast("Notatet er gjort om til symbol."); return; } if (type === "text" && !title && !body) { toast("Tekstnotatet kan ikke være tomt."); return; } selectedFeature.setProperties({ title: title || (type === "sketch" ? "Skisse" : "Notat"), body }); if (type === "text" && selectedFeature.get("bubble")) fitBubbleToText(selectedFeature); saveData(); notesSource.changed(); drawingSource.changed(); hideDetail(); toast("Teksten er oppdatert."); });
...
$("draw-menu-toggle").addEventListener("click", () => { const opening = $("draw-menu").hidden; $("export-menu").hidden = true; $("draw-menu").hidden = !opening; $("draw-menu-toggle").setAttribute("aria-expanded", String(opening)); if (opening) activateDraw(lastDrawKind); });
...
async function showDetail(feature) {
  ...
  const type = feature.get("noteType");
  $("detail-card").classList.toggle("compact-note-editor", type === "text");
  $("detail-card").classList.toggle("compact-symbol-editor", type === "symbol");
  if (type === "text") $("text-editor").append($("delete-object")); else $("edit-geometry").insertAdjacentElement("afterend", $("delete-object"));
  if (type === "symbol") { $("symbol-editor").append($("symbol-actions")); $("symbol-actions").append($("edit-geometry"), $("delete-object")); $("symbol-actions").hidden = false; } else { $("symbol-actions").hidden = true; }
  ...
  if (type === "text" || type === "location" || type === "sketch") { $("edit-note-title").value = feature.get("title") || ""; $("edit-note-text").value = feature.get("body") || ""; if (type === "text") $("edit-note-type").value = "text"; }
