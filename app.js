...
function showDrawContinue(kind) { continueSketchKind = kind; $("continue-draw").textContent = kind === "freehand" ? "Ny frihånd" : kind === "Polygon" ? "Nytt område" : "Ny linje"; $("draw-continue").hidden = false; }
function hideDrawContinue() { continueSketchKind = null; $("draw-continue").hidden = true; }
function activateDraw(kind) { const color = kind === "Polygon" ? "#dc6f45" : $("draw-color").value; hideDrawContinue(); deactivateTool(); toolMode = "draw"; document.querySelector(`[data-draw="${kind}"]`).classList.add("active"); drawInteraction = new ol.interaction.Draw({ source: drawingSource, type: kind === "freehand" ? "LineString" : kind, freehand: kind === "freehand", style: new ol.style.Style({ stroke: new ol.style.Stroke({ color, width: 4 }) }) }); drawInteraction.on("drawend", (event) => { event.feature.setProperties({ id: uid(), noteType: "sketch", sketchKind: kind, styleColor: color, title: "Skisse", body: "", createdAt: new Date().toISOString() }); saveData(); deactivateTool(); showDrawContinue(kind); toast("Skissen er lagret. Tekst kan legges til senere."); }); map.addInteraction(drawInteraction); toast(kind === "Polygon" ? "Tegn område og dobbelttrykk for å avslutte." : "Tegn i kartet. Dobbelttrykk for å avslutte."); }
...
$("cancel-tool").addEventListener("click", () => { deactivateTool(); hideDrawContinue(); hideQuickMenu(); toast("Aktivt verktøy er avsluttet."); });
$("continue-draw").addEventListener("click", () => { if (continueSketchKind) activateDraw(continueSketchKind); });
$("finish-draw").addEventListener("click", () => { hideDrawContinue(); toast("Skissetegning er avsluttet."); });
