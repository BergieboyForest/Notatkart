...
function utm32Projection() { const epsg = "EPSG:25832"; if (!ol.proj.get(epsg)) { proj4.defs(epsg, "+proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs"); ol.proj.proj4.register(proj4); } return epsg; }
...
function transformUtm32(coordinate) { return ol.proj.transform(coordinate, utm32Projection(), projection); }
...
function dxfPoints(properties) { const points = []; let x; properties.forEach(([group, value]) => { if (group === 10) x = Number(value); if (group === 20 && Number.isFinite(x)) { points.push(transformUtm32([x, Number(value)])); x = undefined; } }); return points; }
async function importDxfFile(file) {
  ...
  for (let i = 0; i < pairs.length;) {
    ...
    if (entity === "POINT") { const x = dxfNumber(properties, 10), y = dxfNumber(properties, 20); if (Number.isFinite(x) && Number.isFinite(y)) add(new ol.geom.Point(transformUtm32([x, y])), layer); }
    if (entity === "LINE") { const x1 = dxfNumber(properties, 10), y1 = dxfNumber(properties, 20), x2 = dxfNumber(properties, 11), y2 = dxfNumber(properties, 21); if ([x1, y1, x2, y2].every(Number.isFinite)) add(new ol.geom.LineString([transformUtm32([x1, y1]), transformUtm32([x2, y2])]), layer); }
    ...
    if (entity === "POLYLINE") { const flags = dxfNumber(properties, 70) || 0; const points = []; let cursor = next; while (pairs[cursor]?.[0] === 0 && pairs[cursor]?.[1] === "VERTEX") { let vertexEnd = cursor + 1; while (vertexEnd < pairs.length && pairs[vertexEnd][0] !== 0) vertexEnd += 1; const vertex = pairs.slice(cursor + 1, vertexEnd); const x = dxfNumber(vertex, 10), y = dxfNumber(vertex, 20); if (Number.isFinite(x) && Number.isFinite(y)) points.push(transformUtm32([x, y])); cursor = vertexEnd; } if (points.length > 1) add((flags & 1) ? new ol.geom.Polygon([[...points, points[0]]]) : new ol.geom.LineString(points), layer); i = cursor; continue; }
    if (entity === "CIRCLE") { const x = dxfNumber(properties, 10), y = dxfNumber(properties, 20), radius = dxfNumber(properties, 40); if ([x, y, radius].every(Number.isFinite)) { const center = transformUtm32([x, y]); const east = transformUtm32([x + radius, y]); add(new ol.geom.Circle(center, Math.hypot(east[0] - center[0], east[1] - center[1])), layer); } }
    i = next;
  }
...
function activateDraw(kind) { const color = kind === "Polygon" ? "#dc6f45" : $("draw-color").value; deactivateTool(); toolMode = "draw"; document.querySelector(`[data-draw="${kind}"]`).classList.add("active"); drawInteraction = new ol.interaction.Draw({ source: drawingSource, type: kind === "freehand" ? "LineString" : kind, freehand: kind === "freehand", style: new ol.style.Style({ stroke: new ol.style.Stroke({ color, width: 4 }) }) }); drawInteraction.on("drawend", (event) => { event.feature.setProperties({ id: uid(), noteType: "sketch", sketchKind: kind, styleColor: color, title: "Skisse", body: "", createdAt: new Date().toISOString() }); saveData(); deactivateTool(); showDetail(event.feature); requestAnimationFrame(() => $("edit-note-text").focus()); toast("Objektet er klart. Knytt eventuelt tekst til det i panelet."); }); map.addInteraction(drawInteraction); toast(kind === "Polygon" ? "Tegn område og dobbelttrykk for å avslutte." : "Tegn i kartet. Dobbelttrykk for å avslutte."); }
...
function addSymbol(coordinate) { if (!pendingSymbol) return; const name = symbolNames[pendingSymbol.type]; notesSource.addFeature(new ol.Feature({ geometry: new ol.geom.Point(coordinate), id: uid(), noteType: "symbol", symbolType: pendingSymbol.type, symbolText: pendingSymbol.text, rotation: 0, title: name, body: pendingSymbol.text || "Kartfestet symbol", createdAt: new Date().toISOString() })); saveData(); toast(`${name} er lagret på kartpunktet.`); pendingSymbol = null; $("symbol-type").value = ""; $("symbol-text").value = ""; }
...
$("symbol-type").addEventListener("change", (event) => { if (!event.target.value) return; pendingSymbol = { type: event.target.value, text: $("symbol-text").value.trim() }; selectPlacement("symbol"); });
...
async function importFile(file) { const name = file.name.toLowerCase(); if (name.endsWith(".dxf")) return importDxfFile(file); if (name.endsWith(".xml")) return importLandXmlFile(file); if (name.endsWith(".zip")) return importZipFile(file); throw new Error("Filtypen støttes ikke. Velg DXF, LandXML eller ZIP."); }
